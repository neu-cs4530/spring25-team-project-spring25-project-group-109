 ...
 *
 * @param {string} followerUsername - The username of the user that is following.
 * @param {string} followeeUsername - The username of the user that is being followed.
 * @returns {Promise<UserResponse>} - Resolves with the updated user object that is following or an error message.
 */
export const followUser = async (
  ...
  try {
    ...
    const follower: SafeDatabaseUser | null = await UserModel.findOne({
      username: followerUsername,
    });
    const followee: SafeDatabaseUser | null = await UserModel.findOne({
    ...
    const updatedFollower: SafeDatabaseUser | { error: string } = await updateUser(
      followerUsername,
      { following: [...follower.following, followeeUsername] },
    );
    const updatedFollowee: SafeDatabaseUser | { error: string } = await updateUser(
      followeeUsername,
      { followers: [...followee.followers, followerUsername] },
    );

    if ('error' in updatedFollower || 'error' in updatedFollowee) {
      throw Error('Failed to follow.');
    }

    return updatedFollower;
  } catch (error) {
    return { error: `Error occurred when following: ${error}` };
  }
};

/**
 * Unollows the given user.
 *
 * @param {string} followerUsername - The username of the user that is unfollowing.
 * @param {string} followeeUsername - The username of the user that is being unfollowed.
 * @returns {Promise<UserResponse>} - Resolves with the updated user object that is unfollowing or an error message.
 */
export const unfollowUser = async (
  ...
  try {
    ...
    const updatedFollower: SafeDatabaseUser | { error: string } = await updateUser(
      followerUsername,
      {
        following: follower.following.filter(user => user !== followeeUsername),
      },
    );
    const updatedFollowee: SafeDatabaseUser | { error: string } = await updateUser(
      followeeUsername,
      {
        followers: followee.followers.filter(user => user !== followerUsername),
      },
    );
    ...
    return { error: `Error occurred when unfollowing: ${error}` };
  }
};
